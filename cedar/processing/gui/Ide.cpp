...
void cedar::proc::gui::Ide::duplicateSelected()
{
  ...
  // determine the position offset of the duplicates as the average of the posit
  // ions of all selected elements
...
void cedar::proc::gui::Ide::copy()
{
  ...
  //Debug: Output to bin files
  boost::property_tree::write_json("Copy: Group Values.json", groupNode);
  ...
  if (!AllConnectionsNode.empty())
  {
    ...
    //Remove the connections that are not "selected" (if the 2 nodes of the connection are NOT in the selection then remove connection)
  ...
  //Creating stringStream and writing our rootNode to it (converted to Json)
  ...
  //Set created mimedate to json and fill it with our jsonQByteArray
...
void cedar::proc::gui::Ide::paste()
{
  ...
  //Get Mouse Pos and convert it to scene coordinates
  ...
  ////Get Json from Global Clipboard and put it into string
  ...
  for (auto &stepsPair:stepsTree)
  {
    ...
    rootNode.add_child("meta", metaNode);
    rootNode.add_child("steps", singleStepValue);
    rootNode.add_child("ui", singleUiValue);
    rootNode.add_child("connections", emptyNode);
  ...
  for(auto &groupPair:groupTree)
  {
    ...
    rootNode.add_child("meta", metaNode);
    rootNode.add_child("steps", emptyNode);
    rootNode.add_child("ui", emptyNode);
  ...
  if (connectionsTree)
  {
    ...
    //Debug only
    boost::property_tree::write_json("Pasted: connections.json", connectionsOnlyRoot);

    std::stringstream stringstream;
    boost::property_tree::write_json(stringstream, connectionsOnlyRoot);
